...
const PRESETS = [
  ...
  },
  {
    name: "Gravel 2x",
    chainrings: [46, 30],
    cassette: "10,12,14,16,18,21,24,28,32,36",
  },
  {
    name: "Gravel 1x",
    chainrings: [42],
...
const CASSETTE_PRESETS: Record<
  CassetteSpeed,
  { label: string; teeth: string; note?: string }[]
> = {
  10: [
  ...
  12: [
    ...
    {
      label: "10-28",
      teeth: "10,11,12,13,14,15,17,19,21,23,25,28",
      note: "SRAM",
    },
    {
      label: "10-36",
      teeth: "10,11,12,13,15,17,19,22,25,28,32,36",
      note: "SRAM",
    },
  ],
};
...
export default function GearRatioScreen() {
  ...
  const [cassetteStr, setCassetteStr] = useState(
  ...
  const allSpeeds = chainrings.flatMap((cr) =>
    cassette.map((sp) => speedAt(cr, sp)),
  );
    ...
    <SafeAreaView style={s.safe} edges={["bottom"]}>
      ...
      >
        ...
        <Text style={s.subtitle}>
          Speed for every gear combo at any cadence.
        </Text>
        ...
        >
          {PRESETS.map((p) => (
            <TouchableOpacity
              key={p.name}
              style={[s.presetChip, preset === p.name && s.presetChipActive]}
              onPress={() => applyPreset(p)}
            >
              <Text
                style={[s.presetText, preset === p.name && s.presetTextActive]}
              >
                {p.name}
              </Text>
            </TouchableOpacity>
        ...
        <View style={s.card}>
          ...
          <View style={s.rowWrap}>
            {([10, 11, 12] as CassetteSpeed[]).map((sp) => (
              <TouchableOpacity
                key={sp}
                style={[s.speedTab, cassetteSpeed === sp && s.speedTabActive]}
                onPress={() => setCassetteSpeed(sp)}
              >
                <Text
                  style={[
                    s.speedTabText,
                    cassetteSpeed === sp && s.speedTabTextActive,
                  ]}
                >
                  {sp}-spd
                </Text>
          ...
          <View style={[s.rowWrap, { marginTop: 8 }]}>
            {CASSETTE_PRESETS[cassetteSpeed].map((p) => (
              <TouchableOpacity
                key={p.label}
                style={[s.casChip, cassetteStr === p.teeth && s.casChipActive]}
                onPress={() => {
                  setCassetteStr(p.teeth);
                  setPreset("");
                }}
              >
                <Text
                  style={[
                    s.casChipText,
                    cassetteStr === p.teeth && s.casChipTextActive,
                  ]}
                >
                  {p.label}
                  {p.note ? ` (${p.note})` : ""}
        ...
        <View style={s.card}>
          ...
          <View style={s.rowWrap}>
            {WHEEL_SIZES.map((w) => (
              <TouchableOpacity
                key={w.label}
                style={[s.chip, rimLabel === w.label && s.chipActive]}
                onPress={() => setRimLabel(w.label)}
              >
                <Text
                  style={[s.chipText, rimLabel === w.label && s.chipTextActive]}
                >
                  {w.label}
                </Text>
              </TouchableOpacity>
          ...
          <View style={s.rowWrap}>
            {TIRE_WIDTHS.map((mm) => (
              <TouchableOpacity
                key={mm}
                style={[s.chip, tireMm === mm && s.chipActive]}
                onPress={() => setTireMm(mm)}
              >
                <Text style={[s.chipText, tireMm === mm && s.chipTextActive]}>
                  {mm}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        <View style={s.card}>
          ...
          >
            ...
            <View style={s.miniToggle}>
              {(["mph", "kmh"] as const).map((u) => (
                <TouchableOpacity
                  key={u}
                  style={[s.miniBtn, units === u && s.miniBtnActive]}
                  onPress={() => setUnits(u)}
                >
                  <Text style={[s.miniText, units === u && s.miniTextActive]}>
                    {u === "mph" ? "mph" : "km/h"}
                  </Text>
                </TouchableOpacity>
              ))}
          ...
          <View style={s.cadRow}>
            <TouchableOpacity
              style={s.cadBtn}
              onPress={() => setCadence((c) => Math.max(50, c - 5))}
            >
              <Text style={s.cadBtnText}>−5</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={s.cadBtn}
              onPress={() => setCadence((c) => Math.max(50, c - 1))}
            >
            ...
            <TouchableOpacity
              style={s.cadBtn}
              onPress={() => setCadence((c) => Math.min(130, c + 1))}
            >
              <Text style={s.cadBtnText}>+1</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={s.cadBtn}
              onPress={() => setCadence((c) => Math.min(130, c + 5))}
            >
              <Text style={s.cadBtnText}>+5</Text>
            </TouchableOpacity>
          </View>
        ...
        {cassette.length > 0 && chainrings.length > 0 && (
          <>
            <Text style={s.groupLabel}>
              Speed at {cadence} RPM ({unitLabel}) — ⚠ = cross-chain
            </Text>
            <ScrollView
              horizontal
              ...
              <View>
                ...
                {chainrings.map((cr, crIdx) => (
                  <View key={cr + "-" + crIdx} style={s.tableRow}>
                    <View
                      style={[s.tdLabel, { width: LABEL_W, height: ROW_H }]}
                    >
                      <Text style={s.tdLabelText}>{cr}T</Text>
                      <Text style={s.tdLabelSub}>
                        {(cr / cassette[0]).toFixed(1)}–
                        {(cr / cassette[cassette.length - 1]).toFixed(1)}
                      </Text>
                    </View>
                        ...
                        <View
                          ...
                          style={[
                            s.tdCell,
                            {
                              ...
                              opacity: xchain ? 0.45 : 1,
                            },
                          ]}
                        >
                          <Text style={[s.tdText, { color: heatTextColor(p) }]}>
                            {speed.toFixed(1)}
                          </Text>
            ...
            <View style={[s.rowWrap, { marginTop: 12 }]}>
              <StatPill
                label="Lowest"
                value={`${minS.toFixed(1)} ${unitLabel}`}
                color="#4ade80"
              />
              <StatPill
                label="Top"
                value={`${maxS.toFixed(1)} ${unitLabel}`}
                color="#f87171"
              />
              <StatPill
                label="Range"
                value={`${(maxS / minS).toFixed(1)}×`}
                color="#60a5fa"
              />
              <StatPill
                label="Circ."
                value={`${(circumM * 100).toFixed(1)} cm`}
                color="#c084fc"
              />
            </View>
            <Text style={s.note}>
              ⚠ Cross-chaining (big ring + biggest cogs, or small ring +
              smallest cogs) causes extra chain wear.
            </Text>
          </>
        )}
      </ScrollView>
