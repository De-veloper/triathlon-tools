      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          <Text style={styles.subtitle}>
            Estimate your climb time from power, or find the watts needed to hit a target time.
          </Text>
          ...
          <View style={styles.toggleBar}>
            ...
            <View style={[styles.segmentRow, styles.unitRow]}>
              {(['metric', 'imperial'] as UnitSystem[]).map((u) => (
                <TouchableOpacity
                  key={u}
                  style={[styles.segmentBtn, units === u && styles.segmentBtnGrey]}
                  onPress={() => setUnits(u)}
                >
                  <Text style={[styles.segmentText, units === u && styles.segmentTextGrey]}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.climbsScroll}>
            {FAMOUS_CLIMBS.map((c) => {
              const d = units === 'imperial' ? `${(c.distance / 1.60934).toFixed(1)}mi` : `${c.distance}km`;
              return (
                <TouchableOpacity key={c.name} style={styles.climbChip} onPress={() => loadClimb(c)}>
                  <Text style={styles.climbName}>{c.name}</Text>
                  <Text style={styles.climbStat}>{d} @ {c.gradient}%</Text>
                </TouchableOpacity>
              );
            })}
          </ScrollView>
                ...
                <View>
                  ...
                  <View style={styles.targetTimeRow}>
                    ...
                    <TextInput
                      ...
                      keyboardType="number-pad"
                      value={targetSec}
                      onChangeText={(v) => setTargetSec(String(Math.min(parseInt(v) || 0, 59)))}
                      placeholder="sec"
                      placeholderTextColor="#999"
                    />
                  </View>
              ...
              {distKm > 0 && gradePct > 0 && (
                <View style={styles.elevRow}>
                  <Text style={styles.elevText}>
                    📏 Elevation gain: <Text style={{ fontWeight: '700' }}>{Math.round(elevationM)}m</Text>
          ...
          {hasResult && (
            <View style={styles.resultCard}>
              ...
              <View style={styles.resultGrid}>
                {timeResult && (
                  <>
                    <ResultTile label="Estimated Time" value={formatTime(timeResult.timeS)} highlight />
                    <ResultTile
                      label="Speed"
                    ...
                    />
                    <ResultTile label="W/kg" value={`${timeResult.wkg.toFixed(2)} w/kg`} />
                    <ResultTile label="VAM" value={`${Math.round(timeResult.vam)} m/h`} />
                  </>
                )}
                {powerResult && (
                  <>
                    <ResultTile label="Required Power" value={`${Math.round(powerResult.reqPower)} W`} highlight />
                    <ResultTile label="W/kg" value={`${powerResult.wkg.toFixed(2)} w/kg`} />
                    <ResultTile
                      label="Speed"
                      value={units === 'imperial'
                    ...
                    />
                    <ResultTile label="VAM" value={`${Math.round(powerResult.vam)} m/h`} />
                  </>
                )}
              </View>
              ...
              {vam > 0 && (() => {
                ...
                return (
                  <View style={styles.vamRow}>
                    <Text style={[styles.vamValue, { color: lvl.color }]}>VAM {Math.round(vam)} m/h</Text>
...
const styles = StyleSheet.create({
  ...
  toggleBar: { flexDirection: 'row', gap: 10, marginBottom: 16, flexWrap: 'wrap' },
  segmentRow: { flexDirection: 'row', flex: 1, backgroundColor: '#E8EDF3', borderRadius: 10, padding: 3 },
  ...
  climbsLabel: { fontSize: 11, fontWeight: '600', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 },
  ...
  inputCard: {
    ...
    borderWidth: 1, borderColor: '#E2E8F0', padding: 16, marginBottom: 16,
  },
  inputGrid: { gap: 14 },
  inputLabel: { fontSize: 11, fontWeight: '600', color: '#64748b', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 6 },
  input: {
    backgroundColor: '#fff', borderWidth: 1.5, borderColor: '#D8DDE6',
  ...
  vamRow: { flexDirection: 'row', alignItems: 'center', marginTop: 12, paddingTop: 12, borderTopWidth: 1, borderTopColor: '#E8EDF3' },
