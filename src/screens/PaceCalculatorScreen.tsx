...
export default function PaceCalculatorScreen() {
  ...
  const [solvedPaceSecPerKm, setSolvedPaceSecPerKm] = useState<number | null>(null);
  ...
  function calculate() {
    ...
    if (solveFor === 'time') {
      ...
      if (isNaN(distKm) || distKm <= 0) { setError('Enter a valid distance.'); return; }
      const secPerKm = paceToSecPerKm(paceStr, paceUnit);
      if (isNaN(secPerKm) || secPerKm <= 0) { setError('Enter a valid pace.'); return; }
      ...
      if (isNaN(totalSec) || totalSec <= 0) { setError('Enter a valid time (e.g. 1:30:00).'); return; }
      const secPerKm = calcPace(distKm, totalSec);
      setSolvedPaceSecPerKm(secPerKm);
      setResult(`${sport === 'bike' ? 'Speed' : 'Pace'}: ${formatPace(secPerKm, paceUnit)}`);
    } else {
      setSolvedPaceSecPerKm(null);
      const totalSec = timeToSeconds(timeStr);
      if (isNaN(totalSec) || totalSec <= 0) { setError('Enter a valid time.'); return; }
      const secPerKm = paceToSecPerKm(paceStr, paceUnit);
      if (isNaN(secPerKm) || secPerKm <= 0) { setError('Enter a valid pace.'); return; }
        ...
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.segmentRow}>
            {(Object.keys(SPORT_LABELS) as Sport[]).map((s) => (
              <TouchableOpacity
                key={s}
                style={[styles.segmentBtn, sport === s && styles.segmentBtnActive]}
                onPress={() => handleSportChange(s)}
              >
                <Text style={[styles.segmentText, sport === s && styles.segmentTextActive]}>
                  {SPORT_LABELS[s]}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <View style={styles.segmentRow}>
            {(['time', 'pace', 'distance'] as SolveFor[]).map((sf) => (
              <TouchableOpacity
                key={sf}
                style={[styles.segmentBtn, solveFor === sf && styles.segmentBtnActive]}
                onPress={() => { setSolveFor(sf); setResult(''); setError(''); }}
              >
                <Text style={[styles.segmentText, solveFor === sf && styles.segmentTextActive]}>
                  {sf === 'pace' && sport === 'bike' ? 'Speed' : sf.charAt(0).toUpperCase() + sf.slice(1)}
                </Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          {solveFor !== 'distance' && (
            <View style={styles.inputGroup}>
              <View style={styles.labelRow}>
                <Text style={[styles.label, { marginTop: 0, marginBottom: 0 }]}>Distance</Text>
          ...
          {solveFor !== 'pace' && (
            <View style={styles.inputGroup}>
              <Text style={styles.label}>{sport === 'bike' ? 'Speed' : 'Pace'}</Text>
              <View style={styles.paceRow}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.unitScroll}>
                  {units.map((u) => (
                    <TouchableOpacity
                      key={u}
                      style={[styles.unitBtn, paceUnit === u && styles.unitBtnActive]}
                      onPress={() => setPaceUnit(u)}
                    >
                      <Text style={[styles.unitText, paceUnit === u && styles.unitTextActive]}>{u}</Text>
                    </TouchableOpacity>
                  ))}
                </ScrollView>
              </View>
          ...
          {solveFor === 'pace' && (
            <View style={styles.inputGroup}>
              <Text style={styles.label}>{sport === 'bike' ? 'Speed unit' : 'Result unit'}</Text>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      if (solvedPaceSecPerKm !== null) {
                        setResult(`${sport === 'bike' ? 'Speed' : 'Pace'}: ${formatPace(solvedPaceSecPerKm, u)}`);
                      }
                    }}
                  >
                    <Text style={[styles.unitText, paceUnit === u && styles.unitTextActive]}>{u}</Text>
...
const styles = StyleSheet.create({
  ...
  segmentRow: { flexDirection: 'row', backgroundColor: '#E8EDF3', borderRadius: 10, padding: 3, marginBottom: 4 },
